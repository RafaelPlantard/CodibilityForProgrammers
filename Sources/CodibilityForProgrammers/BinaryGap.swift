...
final class BinaryGap {
    func solution(_ n: Int) -> Int {
        let binaryReversed = String(n, radix: 2, uppercase: true).reversed()
        let binary = String(binaryReversed)
        let binaryReversedAsInt = Int(binary) ?? 0
        let binaryWithoutZerosAfterLastOne = String("\(binaryReversedAsInt)".reversed())
